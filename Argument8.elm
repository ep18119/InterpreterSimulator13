port module Argument8 exposing (..)
...
init flags = (
    ...
    , getASTRoots = 
        ...
        , ASTxy
            ...
            ( ASTne
                { getBrickType = BasicBrick
                , getBrickCommand = CommandTurnSet
                , getBrickArgument = {disp = "n = ", value = ""}
                } Nil Nil
            )  
        , ASTxy
        ...
        , ASTxy
            ...
            ( ASTne
                { getBrickType = BasicBrick
                , getBrickCommand = CommandChangeX
                , getBrickArgument = {disp = "n = ", value = ""}
                } Nil Nil
            )
        , ASTxy
        ...
        , ASTxy
            ...
            ( ASTne
                { getBrickType = BasicBrick
                , getBrickCommand = CommandChangeY
                , getBrickArgument = {disp = "n = ", value = ""}
                } Nil Nil
            )
        , ASTxy
            ...
            ( ASTne
                { getBrickType = BasicBrick
                , getBrickCommand = CommandDownPen
                , getBrickArgument = {disp = "", value = ""}
                } Nil Nil
            )        
        , ASTxy
            ...
            ( ASTne
                { getBrickType = BasicBrick
                , getBrickCommand = CommandUpPen
                , getBrickArgument = {disp = "", value = "test"}
                } Nil Nil
            )    
         , ASTxy
            (50, 650)
            ( ASTne
                ...
                , getBrickCommand = CommandIf
                , getBrickArgument = {disp = "X =", value = ""}
                } Nil Nil
            )
         , ASTxy
            (200, 650)
...
update msg model = 
    case msg of
        ...
        MsgStartDnD ast event ->
            (startDnDxy ast event model, Cmd.none)
        MsgLetMeRoot ast xy event ->
            (letMeRoot ast event model |> startDnD xy event, Cmd.none)
        MsgMoveUs event ->
...
startDnD : Position -> Mouse.Event -> Model -> Model
startDnD xy event model = 
...
attachMe (ASTxy xy (ASTne node bottom right)) xy1 event model = 
  if event.button /= Mouse.MainButton then model
  else
    let 
...
view model =
        ...
        [
          div[][ button [ onClick OpenWindow ][ text " ウィンドウを表示 "] ]
        , div[][ button [ onClick Encoding ][ text " プログラムを送信 "] ]
        , model.getASTRoots
          |> List.indexedMap (\index astxy -> (String.fromInt index, viewASTxy model astxy)) 
          |> Keyed.node "div" []
    ]
...
basicBrickSvg size command argument argCode xy = div [] 
  ...
  , if not (String.isEmpty argument.disp) then
      div [ Html.Attributes.style "position" "absolute"
          ...
          , stopPropagationOn "mouseup" (Json.succeed (Dummy,True))
          , stopPropagationOn "mousedown" (Json.succeed (Dummy,True))
          ]
        [ --Html.text argument.disp
